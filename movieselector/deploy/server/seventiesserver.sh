...
VERSION="1"
MOVIE_SERVER_IMAGE="$ECR_URL/seventiesserver:$VERSION.0"
#MOVIE_SELECTOR_IMAGE="$ECR_URL/movieselector"
#MOVIE_UI_IMAGE="$ECR_URL/movieui:$VERSION.0"
STACK_NAME="$PROJECT_NAME-seventiesserver-$VERSION"
...
aws cloudformation deploy \
    ...
    --template-file "$DIR/seventiesserver.yaml" \
    --capabilities CAPABILITY_IAM \
    --parameter-overrides \
    ProjectName=$PROJECT_NAME \
    EnvoyImage=$ENVOY_IMAGE \
    MovieServerImage=$MOVIE_SERVER_IMAGE \
    Version=$VERSION
